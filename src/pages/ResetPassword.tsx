      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-white/60 text-xs">India's Trusted 3D Printing Partner</p>
          </div>
        </div>
      </div>
      <div className="relative z-10">
        ...
        <p className="text-white/75 text-base mb-8 leading-relaxed max-w-sm">Choose a strong password to keep your 3D printing orders and personal data secure.</p>
        <div className="grid grid-cols-1 gap-3">
          {[
            ...
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 backdrop-blur-md border border-white/10">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#f78e00]/20 to-purple-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                <div className="text-white">{f.icon}</div>
              </div>
              <div>
                <p className="text-white font-medium text-sm mb-0.5">{f.title}</p>
                <p className="text-white/60/70 text-xs leading-relaxed">{f.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="relative z-10">
        <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 shadow-xl">
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <div><p className="text-white text-sm">Priya Patel</p><p className="text-white/60/70 text-xs">Product Designer, Bangalore</p></div>
          </div>
        </div>
      </div>
    </>
    ...
    return (
      <AuthLayout>
        <div className="w-full max-w-md mx-auto text-center">
          ...
          <Link to="/login" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg font-semibold text-white text-sm transition-all duration-200" style={{ background: 'linear-gradient(135deg, #f98f00, #f98f00)' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required autoComplete="new-password" placeholder="Create a strong password" className="w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-400 transition-all duration-200 focus:outline-none focus:border-[#f78e00] focus:ring-4 focus:ring-[#f78e00]/10 hover:border-gray-400" />
          ...
          <div>
            ...
            <div className="relative">
              <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400"><Lock size={16} /></div>
              <input type={showPw ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required autoComplete="new-password" placeholder="Re-enter your password" className={`w-full pl-10 pr-10 py-2.5 border rounded-lg text-sm text-gray-900 placeholder-gray-400 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-[#f78e00]/10 hover:border-gray-400 ${confirmPassword ? password === confirmPassword ? 'border-green-300 focus:border-green-500' : 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-[#f78e00]'}`} />
          ...
          <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg font-semibold text-white text-sm transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed shadow-lg shadow-[#f78e00]/20 hover:shadow-xl hover:shadow-[#f78e00]/30 hover:-translate-y-0.5 active:translate-y-0" style={{ background: loading ? '#f98f00' : 'linear-gradient(135deg, #f98f00, #f98f00)' }}>
